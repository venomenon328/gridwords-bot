...
public final class CanonicalGridWordsPublicationService {
    ...
    private CanonicalResultMessage message(GameResultStore.StoredGameResult result,long playerId) {
        ...
        return new CanonicalResultMessage(players.findByDiscordUserId(playerId).orElseThrow().displayName(),GameType.GRIDWORDS,parsed.gameDate(),parsed.outcome(),parsed.duration(),parsed.board().orElseThrow(),summary,complete?OptionalInt.of(summary.personalComplete()):OptionalInt.empty(),perfect?OptionalInt.of(summary.personalPerfect()):OptionalInt.empty(),OptionalInt.empty(),OptionalInt.empty(),"gridwords-result-"+result.id());
    }
}
