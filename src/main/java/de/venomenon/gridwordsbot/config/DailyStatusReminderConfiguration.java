...
class DailyStatusReminderConfiguration {
    @Bean DailyStatusProjector dailyStatusProjector(GameResultStore results, PlayerStore players) { return new DailyStatusProjector(results, players); }
    @Bean @ConditionalOnBean(JDA.class) JdaDailyStatusMessageGateway jdaDailyStatusMessageGateway(JDA jda) { return new JdaDailyStatusMessageGateway(jda); }
    @Bean @ConditionalOnBean(JdaDailyStatusMessageGateway.class) DailyStatusRefreshService dailyStatusRefreshService(DailyStatusProjector projector, DailyStatusStore store, JdaDailyStatusMessageGateway gateway, Clock clock, GridwordsBotProperties properties) { return new DailyStatusRefreshService(projector, store, gateway, clock, properties.schedule().timeZone(), properties.discord().guildId(), properties.discord().channelId()); }
    @Bean @ConditionalOnBean({JdaDailyStatusMessageGateway.class, CanonicalMessageGateway.class})
