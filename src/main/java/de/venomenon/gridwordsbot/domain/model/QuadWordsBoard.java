...
public record QuadWordsBoard(List<String> rows) {
    ...
    public QuadWordsBoard {
        ...
        for (String row : rows) {
            if (row == null || row.codePointCount(0, row.length()) != 5) {
                throw new IllegalArgumentException("each row needs five cells");
            }
    ...
    /** Canonical persistence representation: one five-cell row per line. */
    public String canonicalText() {
        return String.join("\n", rows);
    }

    /** Rehydrates only the canonical line-based representation written by {@link #canonicalText()}. */
    public static QuadWordsBoard fromCanonicalText(String text) {
        Objects.requireNonNull(text, "text");
        return new QuadWordsBoard(List.of(text.split("\\n", -1)));
    }
}
