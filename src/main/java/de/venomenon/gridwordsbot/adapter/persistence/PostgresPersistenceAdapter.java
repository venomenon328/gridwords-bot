...
public class PostgresPersistenceAdapter implements PlayerStore, GameResultStore, SubmissionStore {
    ...
    public StoredPlayer upsert(PlayerUpsert request) {
        ...
        return jdbc.queryForObject("""
                ...
                RETURNING *
                """, PLAYER, request.discordUserId(), request.displayName(), request.active(), request.administrator(), now, now);
    }
    ...
    public StoredGameResult setCanonicalMessageId(long resultId, long canonicalMessageId) {
        ...
        StoredGameResult result = jdbc.queryForObject("UPDATE game_result SET canonical_message_id = ?, updated_at = ?, version = version + 1 WHERE id = ? RETURNING *", RESULT, canonicalMessageId, clock.instant(), resultId);
    ...
    public StoredSubmission register(SubmissionRegistration registration) {
        ...
        int inserted = jdbc.update("""
                ...
                """, registration.sourceMessageId(), registration.guildId(), registration.channelId(), registration.authorPlayerId(), registration.rawMessageContent(), registration.receivedAt(), now);
        if (inserted == 1) {
    ...
    public StoredSubmission storeResult(ResultStorage request) {
        ...
        int changed = jdbc.update("""
                ...
                """, result.id(), clock.instant(), request.sourceMessageId());
    ...
    public boolean transition(long sourceMessageId, SubmissionState expectedState, SubmissionState targetState) {
        return jdbc.update("UPDATE submission SET processing_state = ?, updated_at = ?, version = version + 1 WHERE source_message_id = ? AND processing_state = ?",
                targetState.name(), clock.instant(), sourceMessageId, expectedState.name()) == 1;
    }
    ...
    private StoredGameResult upsertResult(GameResultUpsert request, Instant now) {
        ...
        return jdbc.queryForObject("""
                ...
                parsed.board().map(NormalizedBoard::canonicalText).orElse(null), request.rawShareText(), request.parserVersion(), now, now);
    }
