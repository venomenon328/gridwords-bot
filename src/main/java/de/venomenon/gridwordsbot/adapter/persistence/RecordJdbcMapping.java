...
final class RecordJdbcMapping {
    ...
    static RecordValue value(ResultSet rs, String prefix) throws SQLException {
        ...
        return switch (kind) {
            case ATTEMPTS_AND_DURATION -> new AttemptsDurationRecordValue(
                    rs.getInt(prefix + "attempts"), Duration.ofMillis(rs.getLong(prefix + "duration_millis")));
            case DURATION -> new DurationRecordValue(Duration.ofMillis(rs.getLong(prefix + "duration_millis")));
            case STREAK -> new StreakRecordValue(
                    rs.getInt(prefix + "streak_length"),
                    rs.getObject(prefix + "streak_start_date", LocalDate.class),
                    rs.getObject(prefix + "streak_end_date", LocalDate.class));
        };
    }
    ...
    static RecordSourceReference source(ResultSet rs, String prefix) throws SQLException {
        ...
        return switch (type) {
            case GAME_RESULT -> new RecordSourceReference.GameResult(
                    rs.getLong(prefix + "source_game_result_id"),
                    rs.getLong(prefix + "source_game_result_version"),
                    rs.getLong(prefix + "source_game_player_id"),
                    GameType.valueOf(rs.getString(prefix + "source_game_type")),
            ...
            case STREAK_RUN -> new RecordSourceReference.StreakRun(
                    ...
                    streakOwner(
                            rs.getString(prefix + "source_streak_owner_type"),
                            rs.getObject(prefix + "source_streak_owner_player_id", Long.class)),
    ...
    static RecordStateSnapshot state(ResultSet rs) throws SQLException {
        RecordStateKey key = new RecordStateKey(
                ...
                new de.venomenon.gridwordsbot.domain.record.RecordDefinitionVersion(rs.getString("definition_version")),
                scope(rs));
        return new RecordStateSnapshot(
                key,
                Optional.ofNullable(rs.getObject("holder_player_id", Long.class)),
                value(rs, ""),
                source(rs, ""),
                Optional.ofNullable(instant(rs, "source_game_first_accepted_at")),
                rs.getBoolean("running"),
                new de.venomenon.gridwordsbot.domain.record.RecordLockVersion(rs.getLong("lock_version")),
    ...
    static Object[] stateWriteParameters(RecordStateWrite write) {
        ...
        return new Object[] {
                write.holderPlayerId().orElse(null),
                values.kind(), values.attempts(), values.durationMillis(),
                values.streakLength(), values.streakStart(), values.streakEnd(),
                source.type(), source.gameResultId(), source.gameResultVersion(), source.gamePlayerId(),
                source.gameType(), source.gameDate(),
                write.sourceGameFirstAcceptedAt().map(RecordJdbcMapping::utc).orElse(null),
                source.streakMetric(), source.streakOwnerType(), source.streakOwnerPlayerId(),
                source.streakStartDate(), write.running()
        };
    }
    ...
    static String sourceKey(RecordSourceReference source) {
        return switch (source) {
            case RecordSourceReference.GameResult result -> result.resultId() + ":" + result.resultVersion()
                    + ":" + result.playerId() + ":" + result.game().name() + ":" + result.gameDate();
            case RecordSourceReference.StreakRun streak -> switch (streak.owner()) {
                case RecordSourceReference.StreakRunOwner.Player player -> streak.metric().name()
                        + ":PLAYER:" + player.playerId() + ":" + streak.startDate();
                case RecordSourceReference.StreakRunOwner.Shared ignored -> streak.metric().name()
                        + ":SHARED:" + streak.startDate();
            };
        };
    }
    static RecordSourceReference source(String type, String key) {
        ...
        return switch (RecordSourceType.valueOf(type)) {
            case GAME_RESULT -> new RecordSourceReference.GameResult(
                    ...
                    GameType.valueOf(parts[3]), LocalDate.parse(parts[4]));
            case STREAK_RUN -> new RecordSourceReference.StreakRun(
                    StreakRecordMetric.valueOf(parts[0]),
                    "PLAYER".equals(parts[1])
                            ? new RecordSourceReference.StreakRunOwner.Player(Long.parseLong(parts[2]))
                            : new RecordSourceReference.StreakRunOwner.Shared(),
                    LocalDate.parse(parts[parts.length - 1]));
        };
    }
    private static long parsePersonalScope(String key) {
        return Long.parseLong(key.substring("player:".length()));
    }
    private static RecordSourceReference.StreakRunOwner streakOwner(String type, Long playerId) {
        return "PLAYER".equals(type)
                ? new RecordSourceReference.StreakRunOwner.Player(playerId)
                : new RecordSourceReference.StreakRunOwner.Shared();
    }
    record ValueColumns(
            String kind,
        ...
        static ValueColumns of(RecordValue value) {
            return switch (value) {
                case AttemptsDurationRecordValue attempts -> new ValueColumns(
                        attempts.kind().name(), attempts.attempts(), attempts.duration().toMillis(),
                        null, null, null);
                case DurationRecordValue duration -> new ValueColumns(
                        duration.kind().name(), null, duration.duration().toMillis(),
                        null, null, null);
                case StreakRecordValue streak -> new ValueColumns(
                        streak.kind().name(), null, null, streak.length(),
    ...
    record SourceColumns(
            String type,
            Long gameResultId,
            Long gameResultVersion,
            Long gamePlayerId,
            String gameType,
            LocalDate gameDate,
            String streakMetric,
            String streakOwnerType,
            Long streakOwnerPlayerId,
            LocalDate streakStartDate) {
        static SourceColumns of(RecordSourceReference source) {
            return switch (source) {
                case RecordSourceReference.GameResult game -> new SourceColumns(
                        game.sourceType().name(), game.resultId(), game.resultVersion(), game.playerId(),
                        game.game().name(), game.gameDate(), null, null, null, null);
                case RecordSourceReference.StreakRun streak -> {
                    Long owner = streak.owner() instanceof RecordSourceReference.StreakRunOwner.Player player
                            ? player.playerId() : null;
                    String ownerType = owner == null ? "SHARED" : "PLAYER";
                    yield new SourceColumns(
                            streak.sourceType().name(), null, null, null, null, null,
                            streak.metric().name(), ownerType, owner, streak.startDate());
                }
            };
        }
    }
