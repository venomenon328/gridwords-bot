...
/** Short, token- and lease-fenced PostgreSQL transitions for future bootstrap orchestration. */
public final class PostgresRecordBootstrapStore implements RecordBootstrapStore {
    private final JdbcTemplate jdbc;
    private final Clock clock;

    public PostgresRecordBootstrapStore(JdbcTemplate jdbc, Clock clock) {
    ...
    public RecordBootstrapSnapshot register(RecordBootstrapKey key) {
        ...
        jdbc.update(
                "INSERT INTO record_bootstrap (guild_id,definition_version,bootstrap_state,attempt_count,created_at,updated_at) VALUES (?,?,'OPEN',0,?,?) ON CONFLICT (guild_id,definition_version) DO NOTHING",
                key.guildId(), key.definitionVersion().value(), RecordJdbcMapping.utc(now), RecordJdbcMapping.utc(now));
    ...
    public Optional<RecordBootstrapSnapshot> find(RecordBootstrapKey key) {
        return jdbc.query(
                        "SELECT * FROM record_bootstrap WHERE guild_id=? AND definition_version=?",
                        (rs, row) -> snapshot(rs),
                        key.guildId(),
                        key.definitionVersion().value())
                .stream()
                .findFirst();
    }

    @Override
    public Optional<RecordLeaseClaim> claim(RecordBootstrapKey key, RecordLeaseClaimRequest request) {
    ...
    @Override
    public boolean renewLease(RecordBootstrapKey key, UUID token, RecordLeaseClaimRequest request) {
        return jdbc.update(
                        "UPDATE record_bootstrap SET claim_until=?,updated_at=? WHERE guild_id=? AND definition_version=? AND bootstrap_state='CLAIMED' AND claim_token=? AND claim_until>?",
                        RecordJdbcMapping.utc(request.leaseUntil()),
                        RecordJdbcMapping.utc(request.claimedAt()),
                        key.guildId(),
                        key.definitionVersion().value(),
    ...
    @Override
    public boolean markSucceeded(RecordBootstrapKey key, UUID token, Instant completedAt) {
        return terminal(key, token, completedAt, "SUCCEEDED", null);
    }

    @Override
    public boolean markRetryableFailure(
            RecordBootstrapKey key, UUID token, RecordWorkFailure failure, Instant nextRetryAt) {
        ...
        return jdbc.update(
                        """
                        UPDATE record_bootstrap
                        SET bootstrap_state='RETRYABLE',claim_token=NULL,claim_until=NULL,next_retry_at=?,
    ...
    public boolean markPermanentFailure(
            RecordBootstrapKey key, UUID token, RecordWorkFailure failure, Instant completedAt) {
        if (failure.category() != RecordWorkFailureCategory.PERMANENT) {
    ...
    private boolean terminal(
        ...
        return jdbc.update(
                        """
                        UPDATE record_bootstrap
                        SET bootstrap_state=?,claim_token=NULL,claim_until=NULL,next_retry_at=NULL,
    ...
    private RecordBootstrapSnapshot snapshot(ResultSet rs) throws SQLException {
        ...
        return new RecordBootstrapSnapshot(
                new RecordBootstrapKey(
                        rs.getLong("guild_id"),
                        new de.venomenon.gridwordsbot.domain.record.RecordDefinitionVersion(
