...
public class PostgresDailyStatusInteractionContextQuery implements DailyStatusInteractionContextQuery {
    ...
    @Override public Optional<Context> findCurrent(long guildId, long channelId, long messageId, LocalDate gameDate) {
        ...
        return Optional.of(new Context(jdbc.query("SELECT p.discord_user_id, p.display_name FROM player p JOIN player_participation_period pp ON pp.player_id = p.discord_user_id WHERE pp.active_from <= ? AND (pp.inactive_from IS NULL OR ? < pp.inactive_from) ORDER BY LOWER(p.display_name), p.discord_user_id", (rs, row) -> new Participant(rs.getLong("discord_user_id"), rs.getString("display_name")), gameDate, gameDate)));
    }
}