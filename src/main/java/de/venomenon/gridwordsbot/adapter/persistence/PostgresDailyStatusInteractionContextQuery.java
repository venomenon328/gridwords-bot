...
public class PostgresDailyStatusInteractionContextQuery implements DailyStatusInteractionContextQuery {
    ...
    public PostgresDailyStatusInteractionContextQuery(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    @Override
    public Optional<Context> findCurrent(long guildId, long channelId, long messageId, LocalDate gameDate) {
        Boolean matches = jdbc.queryForObject(
                "SELECT EXISTS (SELECT 1 FROM daily_status_message WHERE guild_id = ? AND channel_id = ? "
                        + "AND game_date = ? AND bot_message_id = ?)",
                Boolean.class,
                guildId,
                channelId,
                gameDate,
        ...
        return Optional.of(new Context(
                participants(GameType.GRIDWORDS, gameDate),
                participants(GameType.QUADWORDS, gameDate)));
    }

    private List<Participant> participants(GameType gameType, LocalDate gameDate) {
        return jdbc.query("""
                ...
                gameType.name(), gameDate, gameDate);
    }
}