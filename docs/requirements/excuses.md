...
**Stand:** 3. August 2026  
**Gültig ab:** Zwischeninkrement 12.1  
**Verbindliches Issue:** #42

Dieses Dokument definiert ein freiwilliges Unterhaltungsfeature für seltene, klar begründete Ergebniskonstellationen. Der Bot bietet dem betroffenen Nutzer drei redaktionell gepflegte Ausreden an. Auswahl und Stilwechsel bleiben ephemer; nur der gewählte Text wird persistiert und in dieselbe kanonische Ergebnisnachricht aufgenommen.

Bei Widersprüchen zu allgemeinen Aussagen über regelbasierte Kommentare in `docs/implementation-plan.md` gilt für Ausreden dieses Dokument.
...
Eine spätere Nutzung persistierter Ausreden in Achievements, Ergebnisabfragen oder Rückblicken ist möglich, aber nicht Bestandteil von Zwischeninkrement 12.1.

## 2. Abhängigkeit von spielbezogener Teilnahme

Die Produktionsumsetzung beginnt erst nach vollständigem Abschluss und Merge von Issue #39 und Zwischeninkrement 10.6.

Ausreden verwenden danach ausschließlich die historisch wirksame Teilnehmermenge des konkreten Spiels am Spieltag:
...
- Ein gültiges Share darf den Spieler gemäß `docs/requirements/game-specific-participation.md` für genau dieses Spiel ab dem Ergebnistag aktivieren und ist dadurch nicht von einem Ausredenangebot ausgeschlossen.
- Eine Teilnahme am jeweils anderen Spiel ist unerheblich.
- Cooldown und Angebotsverlauf werden pro Spieler und Spiel geführt.
- Die zuletzt gewählten Templates werden dagegen spielübergreifend berücksichtigt, damit ein allgemeiner Text nicht unmittelbar bei GridWords und QuadWords wiederholt wird.

Die Dokumentation dieses Features darf vor Abschluss von 10.6 gemergt werden. Kein Produktionspaket von 12.1 darf vor dem Gesamtabschluss von 10.6 umgesetzt werden.
...
`noExclusivePositivePriorityEvent` ist eine Erweiterungsgrenze. Eine spätere positive Rekord- oder Achievement-Reaktion darf das Ausredenangebot unterdrücken. Zwischeninkrement 12.1 führt dafür keine vorauseilende Rekordberechnung ein.
...
Ein persistiertes Angebot verbraucht den Cooldown unabhängig davon, ob es anschließend:

- ausgewählt,
- abgelehnt,
- ignoriert,
- abgelaufen oder
- durch eine Korrektur invalidiert
...
`GRIDWORDS_VERY_SLOW` gilt bei einer gespeicherten Dauer von mindestens `05:00` Minuten.

`04:59` qualifiziert nicht.

### 5.3 QuadWords

#### Sehr lange Spieldauer

`QUADWORDS_VERY_SLOW` gilt bei einer gespeicherten Dauer von mindestens `08:00` Minuten.

`07:59` qualifiziert nicht.
...
Später eingehende Ergebnisse verändern diese historische Angebotsentscheidung nicht. Templates mit diesem Fakt müssen ausdrücklich einen vorläufigen Stand formulieren, zum Beispiel „unter den bisher eingegangenen Ergebnissen“. Sie dürfen niemals behaupten, der Nutzer sei endgültig Tagesletzter.
...
Der MVP verwendet nur eine kleine explizite Whitelist, zum Beispiel:
...
Ein Template ist nur Kandidat, wenn:

- sämtliche `requiresAll`-Fakten vorliegen,
- keine `excludesAny`-Bedingung vorliegt,
- alle Platzhalter typisiert und vollständig auflösbar sind,
- Spiel und Stil unterstützt werden,
- der Text innerhalb der Discord-Grenzen liegt.
...
Component-IDs enthalten niemals Ausredentext. Sie werden versioniert und vollständig serverseitig validiert, zum Beispiel:
...
- Stil,
- Überschrift oder Label `Ausrede`,
- Anlass,
- Template-ID,
- Themenfamilie,
- Auswahlzeitpunkt.
...
### 14.1 Negativ oder terminal

Bei `NOT_OFFERED`, `DECLINED`, `EXPIRED` oder `INVALIDATED` bleibt der Zustand unverändert.

### 14.2 Noch verfügbar

Bei `AVAILABLE` werden die aktuellen ergebnisabhängigen Fakten neu berechnet. Ursprünglicher Einreichungszeitpunkt und der historische Tagesvergleich bleiben eingefroren.
...
### 14.3 Bereits gewählt

Bei `SELECTED` wird das gewählte Template gegen den aktuellen Ergebniszustand neu geprüft und erneut gerendert.

Die Auswahl bleibt nur bestehen, wenn:

- alle Templatebedingungen weiterhin erfüllt sind und
- der erneut gerenderte Text exakt dem gespeicherten `selected_rendered_text` entspricht.

Andernfalls wechselt der Zustand nach `INVALIDATED`; der Text wird aus der kanonischen Nachricht entfernt. Es gibt keine automatische Ersatzwahl und kein neues Angebot.

Der Bot formuliert eine bereits gewählte Ausrede niemals stillschweigend um.
...
## 17. Nicht Bestandteil von Zwischeninkrement 12.1
...
Zwischeninkrement 12.1 ist fachlich vollständig, wenn mindestens folgende Fälle automatisiert und, wo genannt, real geprüft sind:
