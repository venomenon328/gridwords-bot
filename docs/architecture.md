...
**Verwandte Dokumente:** `anforderungsspezifikation.md`, `implementation-plan.md`, `adr/`
...
Die Architektur trennt daher konsequent:
...
3. **Testbarkeit:** Fachliche Logik, Parser und Zeitregeln müssen ohne Discord und ohne lokal installierte Datenbank testbar sein.
4. **Nachvollziehbarkeit:** Ein Fehler muss anhand persistierter Zustände und strukturierter Logs rekonstruierbar sein.
5. **Einfachheit:** Nur Abstraktionen einführen, die eine konkrete Grenze oder einen konkreten Testnutzen haben.
6. **Erweiterbarkeit innerhalb der Spezifikation:** Version 2 und 3 sollen ohne Architekturbruch ergänzt werden können.
...
- **Dateisystem beziehungsweise später ein einfacher Artefaktspeicher:** hält QuadWords-Rohbilder temporär für maximal 48 Stunden.
...
┌───────────────────────▼──────────────────────────────────────┐
│ Domain                                                       │
│ Spieltag · Ergebnis · Spieltyp · Parse-Ergebnis · Serien     │
...
- `StreakSummary`: aktuelle Spiel- und Lösungsserie
...
- `RawArtifactStore`
- `PlayerDirectory`
- `ApplicationClock` ist nicht nötig; Java `Clock` wird direkt injiziert
...
4. **Kanonisch veröffentlichen:** außerhalb der DB-Transaktion Discord-Nachricht senden.
5. **Transaktion B:** Bot-Message-ID speichern und Zustand `CANONICAL_MESSAGE_PUBLISHED` setzen.
6. **Original löschen:** nur nach erfolgreichem Schritt 5.
7. **Transaktion C:** Löschung/Abschluss persistieren.
8. **Tagesstatus aktualisieren:** Fehler hierbei dürfen das gespeicherte Ergebnis nicht zurückrollen.
...
- Persistenzadapter verwenden pessimistische Sperren nur bei nachgewiesenem Bedarf; zunächst genügen Eindeutigkeitsconstraints und konfliktfeste Upserts/Retry-Behandlung.
...
Der Discord-Adapter entscheidet über Embed-/Textdetails und JDA-Aufrufe. Er darf keine Serien berechnen und keine Ergebnisregeln erfinden.
...
- Aktuelle Serien berücksichtigen die in der Spezifikation definierte Regel für einen noch unvollständigen heutigen Tag.
...
Eine konkrete Scheduler-Technik (`TaskScheduler`, Quartz oder einfache Spring-Scheduling-Komponente) ist implementierungsnah. Für den aktuellen Umfang wird Spring `TaskScheduler` bevorzugt; Quartz wird nur bei einem nachgewiesenen Bedarf eingeführt.
...
Kleine Klassen- oder Methodennamenänderungen benötigen kein ADR.