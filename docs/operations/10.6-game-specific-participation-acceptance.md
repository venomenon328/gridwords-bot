...
Baseline vor Paket 8:

- 501 Standardtests erfolgreich,
- 501 Surefire- und 138 Failsafe-Tests im PostgreSQL-Profil erfolgreich,
- normale GitHub-CI grün,
- Containerworkflow wegen Draft-Status absichtlich übersprungen.

Die finalen Zahlen und Workflow-Links werden nach Abschluss in PR #41 dokumentiert.
...
- zu prüfender Commit eindeutig notiert.

Pro Punkt Ergebnis, relevante Message-IDs und Besonderheiten notieren. Keine Tokens, `.env`-Inhalte oder privaten Serverdaten dokumentieren.

- [ ] Bestehender Zwei-Spiele-Spieler ist nach dem Upgrade fachlich unverändert.
- [ ] Ein gültiges GridWords-Share eines neuen Spielers aktiviert ausschließlich GridWords.
- [ ] Ein gültiges QuadWords-Share eines neuen Spielers aktiviert ausschließlich QuadWords.
- [ ] `/participation join` funktioniert für `gridwords`, `quadwords` und `both`; fehlende Auswahl entspricht `both`.
- [ ] Der Austritt aus genau einem Spiel wirkt ab morgen und lässt das andere Spiel unverändert.
- [ ] Ein globaler Reminder-Opt-out bleibt beim Hinzufügen des zweiten Spiels erhalten.
- [ ] Reminder enthalten keinen Spieler in einer Spielzeile, an deren Spiel er nicht teilnimmt.
- [ ] Der Tagesstatus unterscheidet eindeutig `— nimmt nicht teil` und `⬜ noch nicht eingereicht`.
- [ ] GridWords- und QuadWords-Menüs enthalten jeweils nur ihre historische Teilnehmermenge; Details sind ephemer.
- [ ] Nach Neustart wird der Tagesstatus einschließlich Komponenten korrekt rekonstruiert.
- [ ] Ein Wochenbericht oder kontrollierter Report-Smoke zeigt getrennte Spielnenner und `Nicht teilgenommen` bei null Spieltagen.
- [ ] Korrektur, Replay und Neustart erzeugen keine Duplikate; Originalquellen werden nicht vorzeitig gelöscht.

### Beobachtungsprotokoll

| Punkt | Ergebnis | Beobachtung |
|---|---|---|
| 1–12 | ausstehend | Reale Discord-Abnahme noch nicht durchgeführt. |
...
2. Beide Maven-Verifikationen lokal ausführen.
3. Normale Remote-CI im Draft vollständig grün abwarten.
4. PR #41 auf **Ready for review** setzen.
5. `Build and exercise production image` vollständig grün abwarten.
6. Prüfen, dass der manuelle GHCR-Publish übersprungen blieb.
7. Paket 8 und Issue #39 erst mit belegter vollständiger Abnahme abschließen.
8. PR #41 nicht im Rahmen dieses Abnahmeschritts mergen und kein Image veröffentlichen.
