...
Dieses Runbook beschreibt Bootstrap, Live-Auswertung, Tagesabschluss und Discord-Delivery der produktiven Rekordfunktion. Es ergänzt [troubleshooting.md](troubleshooting.md), den allgemeinen [Backup-/Restoreweg](backup-restore.md) und die fachliche Beschreibung unter [`../product/records.md`](../product/records.md). Die Diagnoseabfragen sind lesend und geben niemals Claim-Tokens aus. `safe_error` wird ausschließlich in der unten gekennzeichneten Fehlerdiagnose angezeigt; die Record-Worker schreiben dort nur stabile, bereinigte Fehlertexte und keine Roh-Exceptions oder Secrets.
...
Ein manueller Rebuild einer Definitionsversion ist eine ausdrücklich autorisierte Wartung und bleibt öffentlich still. Er benötigt ein eigenes, getestetes Wartungswerkzeug beziehungsweise eine nachfolgende Migration; es gibt keinen ungesicherten SQL-Command dafür.

## Live-Auswertung

Eine dauerhaft als `RESULT_STORED` registrierte Submission erzeugt idempotent eine `record_live_evaluation`. Der Worker claimt sie mit Token und Lease, liest und bewertet außerhalb der kurzen Schreibtransaktion und prüft unmittelbar vor State-, Event-, Announcement- und Terminal-Writes erneut Claim, Ergebnisversion und fachliche Generation. Ein Fehler der Rekordauswertung rollt das bereits persistierte Nutzerergebnis nicht zurück.
...
|---|---:|---|
| `RECORD_LIVE_EVALUATION_ENABLED` | `true` | schaltet nur Polling; persistierte Arbeit bleibt erhalten |
| `RECORD_LIVE_EVALUATION_POLL_DELAY` | `PT10S` | mindestens `PT0.001S` |
| `RECORD_LIVE_EVALUATION_LEASE_DURATION` | `PT2M` | größer als null |
| `RECORD_LIVE_EVALUATION_HEARTBEAT_INTERVAL` | `PT30S` | größer als null und kleiner als Lease |
| `RECORD_LIVE_EVALUATION_INITIAL_RETRY_BACKOFF` | `PT10S` | größer als null |
| `RECORD_LIVE_EVALUATION_MAX_RETRY_BACKOFF` | `PT5M` | mindestens initialer Backoff |

Nach einer Änderung den Bot kontrolliert neu starten. Offene, retryfähige oder abgelaufen geclaimte Arbeit wird fortgesetzt.

### Zustände

`record_live_evaluation.evaluation_state` verwendet `OPEN`, `CLAIMED`, `RETRYABLE`, `SUCCEEDED`, `FAILED_PERMANENT` und `SUPERSEDED`. Eine abgelaufene `CLAIMED`-Zeile wird automatisch mit neuem Token übernommen; der alte Worker darf danach weder fachliche noch terminale Writes persistieren.

### Metriken

Die nur auf Loopback erreichbare Managementschnittstelle liefert:
...
Zulässiges niedrig kardinales Tag ist nur `outcome` mit `completed`, `failed_retryable`, `failed_permanent`, `lost_lease` oder `unknown`. IDs, Tokens, Fehlertexte und Exceptionnamen sind keine Metrik-Tags.

### Incident-Hinweise

- Viele `RETRYABLE`: Beginn in Metriken/Logs eingrenzen, Datenbank und Readiness prüfen, Backoff respektieren und keine Zeilen manuell auf Erfolg setzen.
- `FAILED_PERMANENT`: bekannte Invariante anhand sicherer Logs beheben und einen fachlich gültigen Reparaturauftrag erzeugen; Zustand nicht umetikettieren.
- Abgelaufene `CLAIMED`: direkt nach Restart erwartbar; bei dauerhaftem Bestand Scheduler, Uhr, Datenbank und Heartbeat prüfen.
- `lost_lease`: kontrollierter Takeover; Häufungen deuten auf zu kurze Leases, blockierte Datenbank oder Prozesspausen.
- `unknown`: Stacktrace und technische Ursache beheben; der Fehler wird absichtlich nicht als fachlicher Retry maskiert.

## Migration und Application-Rollback

Ausgelieferte Liquibase-Migrationen sind unveränderlich. Korrekturen erfolgen additiv mit neuer Migration und echtem Upgrade-Test. Ein Application-Rollback entfernt weder Liquibase-Migrationen noch Rekordtabellen; Schemaobjekte und Liquibase-Metadaten werden dafür niemals manuell gelöscht.
