...
Eine Reminder-Nachricht wird nur gesendet, wenn mindestens ein Spieler mit offenem Ergebnis Reminder aktiviert hat. Wird sie gesendet, enthält sie weiterhin alle Spieler mit offenen Ergebnissen: Reminder-Opt-in-Spieler werden mit eingeschränkten Allowed Mentions erwähnt, Opt-out-Spieler erscheinen nur mit bereinigtem Anzeigenamen. Sind keine Ergebnisse offen oder bestehen die offenen Kandidaten ausschließlich aus Opt-out-Spielern, wird keine Nachricht versendet und die Stufe terminal als `NO_CANDIDATES` abgeschlossen. Nach einem Neustart wird je Lauf höchstens der jüngste heute fällige Slot nachgeholt; vergangene Spieltage verfallen.

`/reminders on`, `/reminders off` und `/reminders status` antworten ephemer. Die ausführliche Antwort erklärt, dass `aus` echte Mentions verhindert und der bereinigte Klartextname weiterhin in einer gemeinsamen Reminder-Übersicht erscheinen kann, wenn diese wegen mindestens eines anderen Opt-in-Spielers gesendet wird. Angezeigte Reminderzeiten stammen aus der tatsächlich gebundenen Laufzeitkonfiguration und werden nicht als vermeintliche Ist-Werte hart codiert. Im persönlichen `/status` bleibt die Darstellung bewusst auf `Reminder: an/aus` beschränkt.
