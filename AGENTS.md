...
7. `docs/requirements/excuses.md`, wenn die Aufgabe Ausreden, regelbasierte Kommentare, kanonische Ergebnis-Komponenten, Ausredeninteraktionen oder Wiederholungsschutz betrifft
...
### Kontextabhängige Ausreden ab Zwischeninkrement 12.1

- Verbindlich sind `docs/requirements/excuses.md`, ADR 0017 und der Paketplan unter `docs/increments/12.1-contextual-excuses.md`.
- Die Produktionsumsetzung beginnt erst nach vollständigem Abschluss und Merge von Issue #39 beziehungsweise Paket 8 von Zwischeninkrement 10.6.
- Der spätere Implementierungsbranch wird frisch vom dann aktuellen `main` erstellt und nicht auf den laufenden Teilnahmebranch gestapelt.
...
- Das Feature führt kein allgemeines Kommentar-, Event- oder Plugin-Framework vorzeitig ein.
...
- Ausredenänderungen benötigen getrennte Tests für alle absoluten Schwellen und direkte Untergrenzen, den QuadWords-Boardabstand 2 gegenüber 3, boardlose Ergebnisse, mindestens zwei andere Tagesresultate, spielbezogene Teilnehmermengen, Cooldown, Wiederholungsschutz und injizierte Zufallsquelle.
...
- Für Issue #42 darf ausschließlich Paket 0 vor Abschluss von Issue #39 gemergt werden. Die Produktionspakete 1 bis 8 beginnen erst auf einem frischen Branch vom danach aktuellen `main`.
- Für Issue #42 gilt die Reihenfolge aus `docs/increments/12.1-contextual-excuses.md`; keine spätere Interaktions-, Persistenz- oder Katalogstufe provisorisch vorziehen.
- Der Implementierungs-PR für Ausreden bleibt Draft, bis Standardbuild, PostgreSQL-Profil und reale Discord-Abnahme einschließlich Auswahl, Stil-Neuwurf, Ablauf, Neustart und Korrektur vollständig sind.
